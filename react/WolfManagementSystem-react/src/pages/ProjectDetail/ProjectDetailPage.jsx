...
import {
  ArrowLeft, Users, Tag, FolderKanban, MessageSquare, Send,
  Plus, ChevronRight, Clock, AlertCircle, CheckCircle2, Circle,
} from 'lucide-react';
...
import { getProjectDetails } from '@/api/authApi';

// ─── Mock data — issues and chat only ────────────────────────────────────────
const MOCK_ISSUES = [
...
export const ProjectDetailPage = () => {
  ...
  useEffect(() => {
    ...
    getProjectDetails(projectId)
      ...
      .catch(() => setProjectError('Failed to load project. Please try again.'))
      .finally(() => setProjectLoading(false));
  }, [projectId]);

  // ── Derived values (safe-guarded until project is loaded) ──────────────────
          ...
          <div className="flex min-w-0 flex-1 items-center gap-3">
            ...
            <div className="min-w-0">
              ...
              <p className="text-xs text-muted-foreground">
                {project.category} · {project.projectSize ?? project.team?.length ?? 0} member{(project.projectSize ?? project.team?.length ?? 0) !== 1 ? 's' : ''}
              </p>
            </div>
          </div>
                  ...
                  {issues.slice(0, 4).map((issue, idx) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <span
                          className={`shrink-0 rounded-full px-2 py-0.5 text-xs font-medium ${PRIORITY_CONFIG[issue.priority]?.bg} ${PRIORITY_CONFIG[issue.priority]?.color}`}
                        >
                          {issue.priority}
                        </span>
                      </li>
            ...
            <div className="space-y-6">
              ...
              <div className="rounded-xl border bg-card p-6 shadow-sm">
                <p className="mb-4 flex items-center gap-2 text-xs font-semibold uppercase tracking-widest text-muted-foreground">
                  <Users className="h-3.5 w-3.5" /> Team · {project.team?.length ?? 0}
                </p>
            ...
            >
              ...
              <div className="flex-1 overflow-y-auto space-y-1 px-5 py-4">
                {messages.map((msg, idx) => {
                  ...
                  const showAvatar =
                    !isMe &&
                    (idx === 0 || messages[idx - 1]?.sender?.fullName !== msg.sender.fullName);
                  return (
